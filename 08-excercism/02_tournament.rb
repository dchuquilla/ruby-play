...
module Tournament
  DISPLAY = "%-30<team>s | %2<played>s | %2<win>s | %2<draw>s | %2<loss>s | %2<points>s\n"

  def self.tally(input)
    row = { team: 'Team', played: 'MP', win: 'W', draw: 'D', loss: 'L', points: 'P' }
    output = (DISPLAY % row)
  ...
  def self.parse(input, tally = [])
    input.split(/;|\n/).each_slice(3) do |match|
      case match[2]
      when 'win'
        tally << { team: match[0], played: 1, win: 1, draw: 0, loss: 0, points: 3 }
        tally << { team: match[1], played: 1, win: 0, draw: 0, loss: 1, points: 0 }
      when 'draw'
        tally << { team: match[0], played: 1, win: 0, draw: 1, loss: 0, points: 1 }
        tally << { team: match[1], played: 1, win: 0, draw: 1, loss: 0, points: 1 }
      when 'loss'
        tally << { team: match[1], played: 1, win: 1, draw: 0, loss: 0, points: 3 }
        tally << { team: match[0], played: 1, win: 0, draw: 0, loss: 1, points: 0 }
      end
    end
    tally
  end
  ...
  def self.calcultations(tally)
    # debug "tally before #{tally}"
    results = tally.group_by { |match| match[:team] }.transform_values do |match|
      match.each_with_object({ team: '', played: 0, win: 0, draw: 0, loss: 0,
        ...
        sums[:win] += match1[:win].to_i
        sums[:draw] += match1[:draw].to_i
        sums[:loss] += match1[:loss].to_i
        sums[:points] += match1[:points].to_i
      end
    end
    results.map { |match| match.last }
  end
end
